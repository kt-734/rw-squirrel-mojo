...
struct sqrrl__TeamTable(Movable):
    ...
    var keepalive: Set[EntityHandle[sqrrl__TeamTableState]]

    def __init__(out self):
        self.table = Table[sqrrl__TeamTableState](sqrrl__TeamTableState())
        self.keepalive = Set[EntityHandle[sqrrl__TeamTableState]]()
    ...
    def dont_keepalive(mut self, e: EntityHandle[sqrrl__TeamTableState]) -> Bool:
        try:
    ...
    def to_json(self, e: EntityHandle[sqrrl__TeamTableState]) -> String:
        var out = String("{")
    ...
    def from_json(mut self, mut sqrrl__tbl_Person: sqrrl__PersonTable, mut sqrrl__tbl_Employee: sqrrl__EmployeeTable, mut sc: sqrrl__JsonScanner) raises -> EntityHandle[sqrrl__TeamTableState]:
    ...
    def all_from_json(mut self, mut sqrrl__tbl_Person: sqrrl__PersonTable, mut sqrrl__tbl_Employee: sqrrl__EmployeeTable, mut sc: sqrrl__JsonScanner) raises:
        sc.expect_byte(UInt8(ord("[")))
        if not sc.try_consume_byte(UInt8(ord("]"))):
            while True:
                sc.expect_byte(UInt8(ord("[")))
