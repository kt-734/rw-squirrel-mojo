...
struct sqrrl__ProjectTable(Movable):
    ...
    def from_json(mut self, mut sqrrl__tbl_Vendor: sqrrl__VendorTable, mut sc: sqrrl__JsonScanner) raises -> EntityHandle[sqrrl__ProjectTableState]:
    ...
    def all_from_json(mut self, mut sqrrl__tbl_Vendor: sqrrl__VendorTable, mut sc: sqrrl__JsonScanner) raises:
        sc.expect_byte(UInt8(ord("[")))
        if not sc.try_consume_byte(UInt8(ord("]"))):
            while True:
                sc.expect_byte(UInt8(ord("[")))
