...
struct sqrrl__PersonTable(Movable):
    ...
    def for_home(self, value: Address) -> Set[EntityHandle[sqrrl__PersonTableState]]:
    ...
    def count_home(self, value: Address) -> Int:
        return len(self.table.state[].state.home.get_bwd(value))

    def group_by_home(self) -> Dict[Address, Set[EntityHandle[sqrrl__PersonTableState]]]:
        ref buckets = self.table.state[].state.home.all_bwd()
        var out = Dict[Address, Set[EntityHandle[sqrrl__PersonTableState]]]()
        for entry in buckets.items():
            var handles = Set[EntityHandle[sqrrl__PersonTableState]]()
            for id in entry.value:
                handles.add(self.table.handle_for(id))
    ...
    def count_by_home(self) -> Dict[Address, Int]:
        ref buckets = self.table.state[].state.home.all_bwd()
        var out = Dict[Address, Int]()
        for entry in buckets.items():
            out[entry.key] = len(entry.value)
        return out^

    def distinct_home(self) -> Set[Address]:
        var out = Set[Address]()
        for key in self.table.state[].state.home.all_bwd().keys():
            out.add(key)
        return out^
    ...
    def for_job(self, value: EntityHandle[sqrrl__EmployeeTableState]) -> Set[EntityHandle[sqrrl__PersonTableState]]:
    ...
    def count_job(self, value: EntityHandle[sqrrl__EmployeeTableState]) -> Int:
        return len(self.table.state[].state.job.get_bwd(value))

    def group_by_job(self) -> Dict[EntityHandle[sqrrl__EmployeeTableState], Set[EntityHandle[sqrrl__PersonTableState]]]:
        ref buckets = self.table.state[].state.job.all_bwd()
        var out = Dict[EntityHandle[sqrrl__EmployeeTableState], Set[EntityHandle[sqrrl__PersonTableState]]]()
        for entry in buckets.items():
            var handles = Set[EntityHandle[sqrrl__PersonTableState]]()
            for id in entry.value:
                handles.add(self.table.handle_for(id))
    ...
    def count_by_job(self) -> Dict[EntityHandle[sqrrl__EmployeeTableState], Int]:
        ref buckets = self.table.state[].state.job.all_bwd()
        var out = Dict[EntityHandle[sqrrl__EmployeeTableState], Int]()
        for entry in buckets.items():
            out[entry.key] = len(entry.value)
        return out^

    def distinct_job(self) -> Set[EntityHandle[sqrrl__EmployeeTableState]]:
        var out = Set[EntityHandle[sqrrl__EmployeeTableState]]()
        for key in self.table.state[].state.job.all_bwd().keys():
            out.add(key)
        return out^
    ...
    def sqrrl__from_json(mut self, mut sqrrl__tbl_Employee: sqrrl__EmployeeTable, mut sc: sqrrl__JsonScanner) raises -> EntityHandle[sqrrl__PersonTableState]:
    ...
    def sqrrl__all_from_json(mut self, mut sqrrl__tbl_Employee: sqrrl__EmployeeTable, mut sqrrl__temp: List[EntityHandle[sqrrl__PersonTableState]], mut sc: sqrrl__JsonScanner) raises:
        sc.expect_byte(UInt8(ord("[")))
        if not sc.try_consume_byte(UInt8(ord("]"))):
            while True:
                sc.expect_byte(UInt8(ord("[")))
