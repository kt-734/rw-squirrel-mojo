...
struct sqrrl__PersonTable(Movable):
    ...
    def value_eq(self, a: EntityHandle[sqrrl__PersonTableState], b: EntityHandle[sqrrl__PersonTableState]) -> Bool:
        if self.get_name(a) != self.get_name(b):
            return False
        if self.get_home(a) != self.get_home(b):
            return False
        if self.get_job(a) != self.get_job(b):
            return False
        return True
    ...
    def group_by_home(self) -> Dict[Address, List[EntityHandle[sqrrl__PersonTableState]]]:
        ref buckets = self.table.state[].state.home.all_bwd()
        var out = Dict[Address, List[EntityHandle[sqrrl__PersonTableState]]]()
        for entry in buckets.items():
            var handles = List[EntityHandle[sqrrl__PersonTableState]]()
            for id in entry.value:
                handles.append(self.table.handle_for(id))
    ...
    def group_by_job(self) -> Dict[EntityHandle[sqrrl__EmployeeTableState], List[EntityHandle[sqrrl__PersonTableState]]]:
        ref buckets = self.table.state[].state.job.all_bwd()
        var out = Dict[EntityHandle[sqrrl__EmployeeTableState], List[EntityHandle[sqrrl__PersonTableState]]]()
        for entry in buckets.items():
            var handles = List[EntityHandle[sqrrl__PersonTableState]]()
            for id in entry.value:
                handles.append(self.table.handle_for(id))
